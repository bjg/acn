...
int main(int argc, char *argv[])
{
    ...
    double* bpix[MAXSTARS];
    double* cpix[MAXSTARS];
            ...
            for ( j =0; j < sc; j++) {
                ...
                if (fpixel[0] < 1 || fpixel[1] < 1 || lpixel[0] < 1 || lpixel[1] < 1) 
                   ...
                   for (fpixel[2] = 1; fpixel[2] <= anaxes[2]; fpixel[2]++) {
                       ...
                       //printf ("Radius     X        Y          S       I       SkyB       Mag Estimate \n");
                       ...
                       for (radius = MINRADIUS;radius<radiusarray[0];radius++) {
                           skybackground(Bx,By,apix,bpix[j],cpix[j],xguessarray[j], yguessarray[j],boxarray[j],annulusarray[j],dannulusarray[j],radius,&skyb); // Sky background changes as radius moves and pushes out the annulus
...
int centroid(double *x, double *y, double *subrectarray, double *mfarray, double *mbarray, int xpos, int ypos,int boxdims, int threshold) {
 ...
 //           printf ("masterflat = %f ",mfarray[ii*boxdims+b]);
//            printf ("masterbias = %f readval = %f\n",mbarray[ii*boxdims+b],readval);
